...
def loss(depth_gt, depth_pred):
    ...
    # import ipdb
    # ipdb.set_trace()
    depth_pred = torch.where(torch.isnan(depth_pred), torch.full_like(depth_pred, 0), depth_pred)
    mask = (depth_gt > 0).sum(dim=-1).sum(dim=-1).to(depth_pred.device)
    unmask = ~(depth_gt>0)
    depth_pred[unmask] = 0
    depth_gt_avg = depth_gt.sum(dim=-1).sum(dim=-1)/mask
    depth_pred_avg = depth_pred.sum(dim=-1).sum(dim=-1)/mask
    rate = depth_gt_avg/depth_pred_avg
    depth_pred = depth_pred*rate
    abs_error = (depth_pred - depth_gt).abs() / depth_gt
    valid = (abs_error < 0.1).sum(dim=-1).sum(dim=-1) /mask
...
def eval(model, dataloader_eval, post_process=False):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        with torch.no_grad():
            ...
            print("精度是：", loss(gt_depth.squeeze(1).squeeze(-1).cuda(), pred_depth.squeeze(1)).mean().item())
